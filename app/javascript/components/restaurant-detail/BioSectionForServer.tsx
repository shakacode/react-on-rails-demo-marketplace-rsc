// Server-only — renders the markdown bio + story to HTML using the heavy
// markdown stack at request time. Imported only from the RSC tree.
...
export function BioSection({ bio, story, restaurant }: Props) {
  return <BioLayout bio={bio} story={story} restaurant={restaurant} />;
}

// Shared JSX function — used by both the RSC and client variants of this
// section. Lives here as a regular function (not a 'use client' component)
// so importing it from RSC keeps the markdown libs server-only; the
// 'use client' version (BioSection.tsx) imports it via the same path and
// becomes part of the client bundle for SSR/Client variants.
export function BioLayout({ bio, story, restaurant }: Props) {
