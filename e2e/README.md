...
capability token for each invocation; only Playwright's Node request client
receives it. Requests with an Origin, a non-JSON content type, or an invalid
token are rejected, and the gem's unauthenticated state-reset middleware is
never mounted.
